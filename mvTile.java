...
public final class mvTile {
    ...
    public mvTile(int north, int east, int south, int west) {
        this.tflag = -1;
        TileMoldVerticalUtils tu = TileMoldVerticalUtils.getInstance();
        mvTile tile = tu.getTile(north, east, south, west);
    }
...
//        this.WEST = CheckValue(west, "WEST");
    ...
    public int CheckValue(int value, String piname) {
        if (value == 0 || value == 1) {
            ...
            System.err.println("ERROR: MV TILE - Check the value of " + piname.toUpperCase().trim() + " pin!!");
            System.exit(-1);
            return value;
        }
    }
